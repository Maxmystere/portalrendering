...
static void vline(int x, int y1, int y2, int top, int middle, int bottom)
{
	...
	//SDL_RenderCopy(rend, text, NULL, NULL); // Uncomment to see rendering while showing windows
	//SDL_RenderPresent(rend); // Uncomment to see rendering while showing windows
}
...
int main(int argc, char *argv[])
{
	...
	SDL_Window *screen = SDL_CreateWindow("Test", 0, 0, W, H, 0);
	rend = SDL_CreateRenderer(screen, -1, SDL_RENDERER_SOFTWARE);
	text = SDL_CreateTexture(rend, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, W, H);

	SDL_LockTexture(text, NULL, &surfpixels, &argc);
